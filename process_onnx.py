...
def process_sum_nodes(onnx_model):
    ...
    for i, node in enumerate(onnx_model.graph.node):
        if node.op_type == "Sum":
            ...
            k = 0
            while n_inputs > 2:
                # Create a new Sum node for the first two inputs
                new_sum_node = make_node(
                    "Sum",
                ...
                onnx_model.graph.node.insert(i + k, new_sum_node)
                # Replace the first two inputs with the output of the new Sum node
                node.input[0] = f"{node.name}_intermediary_sum_{k}"
                node.input.pop(1)  # Remove the second input
                n_inputs = len(node.input)
                k += 1
    return onnx_model
